...
################################################################################
#Data visualization 
#we can see that the first column is a "character" class (Name)
...
#data(cars)
#Creation of a vector with the names of each column
dim(cars)
...
#Nom we can call the vector of the names of each column

# Here we only want specific data, therefore we need to reemove the exces.
data_del <- c("AWD","Sports", "SUV", "Wagon", "Minivan", "Pickup", "RWD", "Retail",
            "Dealer", "CityMPG", "HighwayMPG")
cars <- cars[ , !(names(cars)%in%data_del)]
...
################################################################################
# FOCUSING ON HORSEPOWER
# TABLES


cars_short %>% slice(c(1:10)) %>%
  kbl(caption = "10 First cars of the Dataset") %>%
  kable_classic(full_width = F, html_font = "Cambria")

cars_ordered <- cars_short[order(cars_short$Horsepower, decreasing=TRUE), ]
cars_ordered%>%slice(c(1:20)) %>%
  kbl(caption = "Top 20 cars with the largest Horsepower") %>%
  kable_classic(full_width = F, html_font = "Cambria")

################################################################################
#Correlation Matrix 
Horsepower <- which(colnames(cars_short) == "Horsepower")
cars_short2<-cars_ordered%>%slice(c(1:15))
ggpairs(cars_short2)

################################################################################
# Partition using the Horsepower
set.seed(100)
ind <- createDataPartition(cars_short$Horsepower,p=0.80,list = F)
...
################################################################################
# PCA
data_del <- c("Name") # NAMES DELATED BECAUSE PCA HAVE ERRORS
cars_short2 <- cars_short2[ , !(names(cars_short2)%in%data_del)]
cars_pca <- princomp(cars_short2, cor=TRUE,)

str(cars_pca)

a1 <- cars_pca$loadings[,1]
a1
a2 <- cars_pca$loadings[,2]
a1%*%a2

#rescaled coefficients
rescaleda1 <- a1 * cars_pca$sdev[1]
rescaleda1
...
plot(cars_pca$sdev^2, xlab="Component number",
     ylab="Component variance", type="l")

screeplot(cars_pca, type="l", main="Screeplot cars")



################################################################################
# NAMES DELATED BECAUSE PCA HAVE ERRORS
#Principal component scores
head(predict(cars_pca)) %>%
  kbl(caption = "Horsepower") %>%
  kable_classic(full_width = F, html_font = "Cambria")

#check that the PCs are uncorrelated (all values are 0)
cor(cars_pca$scores) %>%
  kbl(caption = "Correlation Horsepower") %>%
  kable_classic(full_width = F, html_font = "Cambria")

# 
# pc1.scores <- cars_pca$scores[,1]
# plot(cars$Horsepower, pc1.scores)
...
fviz_pca_ind(cars_pca,
             ...
             title="Cars Horsepower - PCA")

rbind(cars_pca$scores[,1], cars_pca$scores[,2])



#DATA SET ORDERED BY HORSEPOWER
#write.csv(Your DataFrame,"Path to export the DataFrame\\File Name.csv", row.names = FALSE)
write.csv(cars_ordered,"D:/ESEIAAT/Data_Analysis/EXCERCICE-1\\Cars_short.csv", row.names = TRUE)
