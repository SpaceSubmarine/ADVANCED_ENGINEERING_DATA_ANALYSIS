...
# ############################
borrar <- c("Name","AWD","Sports", "SUV", "Wagon", "Minivan", "Pickup", "RWD", "Retail",
...
#La siguiente instrucción pide eliminar rownames

ggpairs(cars_short[, -Horsepower])
...
#cars_short  <- cars_short [-grep(rownames(heptathlon)), ]

#ggpairs(heptathlon[, -score])

cars_prueba<-cars_ordered%>%slice(c(1:10))
#para hacer el PCA hay que limitar los rows sino, peta lol
cars_pca <- princomp(cars_prueba)
str(heptathlon_pca)
...
#hay que poner la primera columna como rownames
cars_pca <- princomp(cars_short)
princomp(~ ., data = cars_short, cor = TRUE)
